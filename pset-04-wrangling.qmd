...
resp_status(cases_raw)
cases_raw <- resp_body_json(cases_raw,simplifyDataFrame = TRUE) 

```

No, all of the data is not there because the API has a default limit of 1000 on the amount of data that can be downloaded. If we want more, we need to specify with the limit parameter.
...
ggplot(aes(x = date, y = cases_per_100k, color = state)) +  
  ...
  labs(
    title = "COVID-19 Cases per 100,000 by Region (2020–2021)",
    x = "Date",
    y = "Cases per 100,000"
  )
```
